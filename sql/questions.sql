...
BEGIN
	...
	FROM dwh.SalesFact INNER JOIN dwh.StoresDim ON dwh.SalesFact.StoreID = dwh.StoresDim.StoreID
...
BEGIN
	...
	FROM dwh.SalesFact
	INNER JOIN dwh.CustomersDim ON dwh.SalesFact.CustomerID = dwh.CustomersDim.CustomerID
...
BEGIN
	...
	INNER JOIN dwh.ProductsDim ON dwh.SalesFact.ProductID = dwh.ProductsDim.ProductID
	INNER JOIN dwh.BrandsDim ON dwh.BrandsDim.BrandID = dwh.ProductsDim.BrandID
...
AS
BEGIN
	SELECT DATENAME(WEEKDAY,TransactionDate) as Day, SUM(TotalAmount) as Sales
	FROM dwh.SalesFact
...
BEGIN
	...
	INNER JOIN dwh.SalesFact sf ON pd.ProductID = sf.ProductID
...
BEGIN
	...
	FROM dwh.SalesFact
	INNER JOIN dwh.StoresDim ON dwh.SalesFact.StoreID = dwh.StoresDim.StoreID
...
BEGIN
	...
	INNER JOIN dwh.ProductsDim pd ON sf.ProductID = pd.ProductId
	INNER JOIN dwh.CategoriesDim cd ON pd.CategoryID = cd.CategoryID
...
DROP PROCEDURE category_by_percent;
DROP PROCEDURE revenue_by_city;
DROP PROCEDURE quarterly_brands;
DROP PROCEDURE customer_demographics;
DROP PROCEDURE sales_by_gender;
DROP PROCEDURE revenue_by_store;
DROP PROCEDURE top_sellers;